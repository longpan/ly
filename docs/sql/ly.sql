...
(
   ...
   vt_course_disease    varchar(100) default NULL comment '???ٲ???',
   ...
   la_bore              varchar(100) default NULL comment 'LA?ھ?',
   lv_bore              varchar(100) default NULL comment 'lv?ھ?',
   lvef_bore            varchar(100) default NULL comment 'LVEF_?ھ?',
   ra_bore              varchar(100) default NULL comment 'RA?ھ?',
   rv_bore              varchar(100) default NULL comment 'RV?ھ?',
   ucg_remarks          varchar(200) default NULL comment 'UCG??ע',
   ecg_type             varchar(100) default NULL comment 'ECG??֧????????',
   electrical_offset    varchar(100) default NULL comment '????ƫ??',
   ...
   inspection_method    varchar(100) default NULL comment '???鷽??',
   ...
   target_position      varchar(100) default NULL comment '?е㲿λ',
   ablation_energy      varchar(100) default NULL comment '??????Դ',
   ablation_judgement   varchar(50) default NULL comment '?ж???????Чָ??',
   ablation_end_point   varchar(100) default NULL comment '?????յ?',
   effective_target_site varchar(100) default NULL comment '??Ч?е?',
   discharge_time       varchar(100) default NULL comment '?ܷŵ?ʱ??',
   xray_exposure_time   varchar(100) default NULL comment 'x?߱???ʱ??',
   ablation_times       int(11) default NULL comment '???ڴ???',
   intraoperative_cable_rate varchar(100) default NULL comment '???е縴??',
   before_heart_rate    varchar(100) default NULL comment '????ǰ????',
   before_vt            varchar(100) default '' comment '????ǰ????',
   before_ront          varchar(100) default NULL comment '????ǰ????',
   before_remarks       varchar(1000) default NULL comment '????ǰ??ע',
...
(
  ...
  invali_danti_arrhythmia_drugs varchar(100) default NULL comment '??ǰ??Ч??????ʧ??ҩ??',
  merger_arrhythmia        varchar(100) default NULL comment '??ǰ?ϲ?????ʧ??',
  imaging_inside_heart     varchar(100) default NULL comment '????????Ӱ',
  induced_way              varchar(100) default NULL comment '?շ???ʽ',
  check_medication         varchar(100) default NULL comment '??????ҩ',
  tachycardia_regulation   varchar(100) default NULL comment '?Ķ????ٷ????Ƿ?????',
  ccl                      varchar(100) default NULL comment '?ܳ???CCL??',
  inspection_method        varchar(100) default NULL comment '???鷽??',
  diastolic_potential      varchar(100) default NULL comment '?????ڵ?λ',
  p_potential_examination  varchar(100) default NULL comment 'P??λ????',
  ablation_procedure       varchar(100) default NULL comment '??????ʽ',
  ablation_lines           varchar(100) default NULL comment '???ھ???',
  target_position          varchar(100) default NULL comment '?е㲿λ',
  ablation_energy          varchar(100) default NULL comment '??????Դ',
  ablation_judgement       varchar(100) default NULL comment '?ж???????Чָ??',
  ablation_end_point       varchar(100) default NULL comment '?????յ?',
  effective_target_site    varchar(100) default NULL comment '??Ч?е?',
  discharge_time           varchar(100) default NULL comment '?ܷŵ?ʱ??',
  xray_exposure_time       varchar(100) default NULL comment 'x?߱???ʱ??',
  ...
  af_ablation_treatment    varchar(100) default NULL,
  before_anticoagulant     varchar(100) default NULL,
  af_type                  varchar(100) default NULL comment '????????',
  af_course_disease        varchar(100) default NULL comment '???ٲ???',
  PRIMARY KEY (id)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
